...
# 프로젝트 개요
*	동국대학교 2025년도 1차 대학원 프로젝트 지원사업으로 진행된 프로젝트임.
...
```mermaid
flowchart LR
  A["PDF 크롤링"] --> B["OCR 전처리"] --> C["텍스트 구조화 및 직무 추론"] --> D["LLM 핵심 역량 산출"] --> E["시각화"]
...
4. LLM 핵심 역량 산출
    ...
    4-3. 출력 파싱 및 정규화
      ...
      *	누락된 역량 항목은 기본값으로 보정하고, 모든 점수를 1~5 범위로 제한함
...
🤖 Large Language Model (LLM)
    ...
    •	LLM (Transformers 기반 Local 모델)
      : Model ID -> "DavidAU/Qwen3-4B-Thinking-2507-Gemini-2.5-Flash-Lite-Preview-Distill-Heretic-Abliterated"
